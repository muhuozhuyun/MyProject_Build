...
namespace MyProject.Products
{
    ...
    public class ProductManager : IDomainService
    {
        ...
        [UnitOfWork]
        public virtual async Task<List<Product>> GetProductsForUserIncludingChildrenAsync(long userId)
        {
            var user = await _userManager.GetUserByIdAsync(userId);
            var organizationUnits = await _userManager.GetOrganizationUnitsAsync(user);
            var organizationUnitCodes = organizationUnits.Select(ou => ou.Code);
            var query = from product in _productRepository.GetAll()
                        join organizationUnit in _organizationUnitRepository.GetAll()
                        on product.OrganizationUnitId equals organizationUnit.Id
